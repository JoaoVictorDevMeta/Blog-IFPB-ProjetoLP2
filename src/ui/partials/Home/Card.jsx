...
export const Card = ({title = '', image, link, type='title-card'}) => {
    ...
    return(
        <div className={'image-card position-relative'} >
            <img src={image || placeholderImage} alt='' className='position-absolute'/>
                { type==='title-card' ? (
                    <div className='card-text position-relative h-100 d-flex align-items-end'>
                        <Link to={`/blog/${link}`} className='fs-3 p-2 pb-4 text-decoration-none fw-medium placeholder-glow'>
                            {title || (
                                <>
